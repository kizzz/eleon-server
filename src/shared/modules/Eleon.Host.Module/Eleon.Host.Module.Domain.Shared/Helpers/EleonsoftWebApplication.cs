...
public class EleonsoftWebApplication
{
  ...
  public static async Task<int> HostWebApplicationAsync<TStartupModule>(string[] args, Func<WebApplicationBuilder, Task>? configureBuilder = null, Func<WebApplication, Task>? configureApp = null) where TStartupModule : IAbpModule
  {
    try
    {
      ...
      configuration.ConfigureDefaultEleonsoftLogger(false);
      ...
      // Don't block startup on database check - do it asynchronously in background
      // This prevents hangs if database is temporarily unavailable
      _ = Task.Run(async () =>
      {
        try
        {
        ...
        {
          // Silently ignore - database check failures shouldn't block startup
        }
      });

      var app = await EleonsoftWebApplication.CreateWebApplicationAsync<TStartupModule>(StartupArgsParser.Raw, configuration, configureBuilder);
