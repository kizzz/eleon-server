...
namespace VPortal.Accounting.Module.Jobs
{
  ...
  public class SuspendAccountTenantJob :
      IDistributedEventHandler<StartingInternalSystemJobExecutionMsg>,
      ITransientDependency
  {
    private readonly IVportalLogger<SuspendAccountTenantJob> logger;
    ...
    public SuspendAccountTenantJob(
        IVportalLogger<SuspendAccountTenantJob> logger,
    ...
    {
      if (eventData.BackgroundJob.Type != AccountingBackgroundJobTypes.SuspendAccountTenant)
      {
        return;
      }
      ...
      try
      {
        ...
        await accountTenantManager.RequestSuspendAccountTenant(accountValueObject, job.Id);
