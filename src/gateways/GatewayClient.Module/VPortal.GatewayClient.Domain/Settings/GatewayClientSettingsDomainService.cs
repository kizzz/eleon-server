...
namespace VPortal.GatewayClient.Domain.Settings
{
    [UnitOfWork]
    public class GatewayClientSettingsDomainService : DomainService, ISingletonDependency
    {
        private static Regex PortRegex = new (@":[0-9]{2,5}");
        private readonly IVportalLogger<GatewayClientSettingsDomainService> logger;
        private readonly IConfiguration configuration;

        public GatewayClientSettingsDomainService(
            IVportalLogger<GatewayClientSettingsDomainService> logger,
        ...
        {
            if (!File.Exists(GatewayClientConfigConsts.AppsettingsPath))
            {
                throw new UserFriendlyException("The configuration file ('appsettings.json') was not found.");
            }

            var jsonText = await File.ReadAllTextAsync(GatewayClientConfigConsts.AppsettingsPath);
            ...
            await File.WriteAllTextAsync(GatewayClientConfigConsts.AppsettingsPath, updatedJson);
        }
