...
namespace VPortal.Core.Infrastructure.Module.Users
{
  ...
  public class CommonUserAppService : TenantManagementAppService, ICommonUserAppService
  {
    ...
    {
      ImportExcelUsersValueObjectDto result = new();
      try
      {
        var reply = await userDomainService.ImportExcelUsers(file);
        result = ObjectMapper.Map<ImportExcelUsersValueObject, ImportExcelUsersValueObjectDto>(reply);
      }
      catch (Exception e)
      {
        logger.Capture(e);
    ...
    public async Task<bool> SetNewPassword(Guid userId, string newPassword)
    {
      ...
      try
      {
        result = await userDomainService.SetNewPassword(userId, newPassword);
      }
      catch (Exception e)
      {
        logger.Capture(e);
    ...
    {
      await _eventBus.PublishAsync(new IgnoreLastUserOtpRequestMsg { UserId = userId });
      return true;
    }
    ...
    {
      return this.identityUserAppService.UpdateRolesAsync(id, input);
    }
    ...
    {
      return this.identityUserAppService.CreateAsync(input);
    }

    public Task<IdentityUserDto> UpdateAsync(Guid id, IdentityUserUpdateDto input)
    {
      return this.identityUserAppService.UpdateAsync(id, input);
    }
