...
namespace VPortal.Accounting.Module.Jobs
{
  ...
  public class CancelAccountTenantJob :
      IDistributedEventHandler<StartingInternalSystemJobExecutionMsg>,
      ITransientDependency
  {
    private readonly IVportalLogger<CancelAccountTenantJob> logger;
    ...
    public CancelAccountTenantJob(
        IVportalLogger<CancelAccountTenantJob> logger,
    ...
    {
      if (eventData.BackgroundJob.Type != AccountingBackgroundJobTypes.CancelAccountTenant)
      {
        return;
      }
      ...
      try
      {
        ...
        await accountTenantManager.RequestCancelAccountTenant(accountValueObject, job.Id);
