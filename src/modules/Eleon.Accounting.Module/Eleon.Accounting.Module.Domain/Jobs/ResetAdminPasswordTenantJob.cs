...
namespace VPortal.Accounting.Module.Jobs
{
  ...
  public class ResetAdminPasswordTenantJob :
      IDistributedEventHandler<StartingInternalSystemJobExecutionMsg>,
      ITransientDependency
  {
    private readonly IVportalLogger<ResetAdminPasswordTenantJob> logger;
    ...
    public ResetAdminPasswordTenantJob(
        IVportalLogger<ResetAdminPasswordTenantJob> logger,
    ...
    {
      if (eventData.BackgroundJob.Type != AccountingBackgroundJobTypes.ResetAdminPasswordAccountTenant)
      {
        return;
      }
      ...
      try
      {
        ...
        await accountTenantManager.RequestResetTenantAdminPassword(accountValueObject, job.Id);
