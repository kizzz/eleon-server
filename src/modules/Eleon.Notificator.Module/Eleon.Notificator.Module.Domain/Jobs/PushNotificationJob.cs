...
public class PushNotificationJobParams
{
...
public class PushNotificationJob : DefaultBackgroundJob, ITransientDependency
{
  ...
  protected override string Type => "SendPushNotification";

  protected override async Task<JobResult> ProcessExecutionAsync(BackgroundJobEto job, BackgroundJobExecutionEto execution)
  {
    var jobParams = _jsonSerializer.Deserialize<PushNotificationJobParams>(execution.StartExecutionParams);
    ...
    var result = new JobResult(true, "Push notifications successfully sent", new List<BackgroundJobTextInfoEto>());
