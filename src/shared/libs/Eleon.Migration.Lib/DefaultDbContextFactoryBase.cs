...
public abstract class DefaultDbContextFactoryBase<TDbContext> : IDesignTimeDbContextFactory<TDbContext> where TDbContext : DbContext
{
  ...
  public static DbContextOptions<TDbContext> CreateOptions()
  {
    ...
    var builder = new DbContextOptionsBuilder<TDbContext>()
        .UseSqlServer(_configuration.GetConnectionString(connectionStringName), opt => opt.UseCompatibilityLevel(_configuration.GetValue("SqlServer:CompatibilityLevel", 120)));
