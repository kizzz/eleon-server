...
using EventBus.LocalBus.Module;
...
[DependsOn(
    ...
    typeof(EventBusLocalBusModule),
    typeof(MassTransitEventBusModule),
  typeof(MassTransitRabbitMqEventBusModule),
    typeof(EleonsoftSdkModule)
    )]
public class EleonHostModule : AbpModule
{
  ...
  public override void ConfigureServices(ServiceConfigurationContext context)
  {
    ...
    Configure<AbpAspNetCoreMvcOptions>(options =>
    {
      ...
      options.ConventionalControllers.Create(typeof(EleonHostModule).Assembly);
    });
    ...
    context.Services.AddAutoMapper(typeof(EleonHostModule));
    ...
    context.Services.AddEleonsoftContentSecurity();

    context.Services.AddEleonsoftPermissions().ForbidApiKeysWithoutAuthorize();
    context.Services.AddEleonsoftExceptionsFilter(true);
    ...
    context.Services.AddTestTelegramNotificationJob(applicationName);
    context.Services.AddSendNotificationJob(applicationName);
    context.Services.AddSessionMiddleware();
  ...
  private void ConfigureLocalizationOptions()
  {
    Configure<AbpVirtualFileSystemOptions>(options =>
    {
      options.FileSets.AddEmbedded<EleonHostModule>();
    });
